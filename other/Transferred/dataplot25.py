...
if Year == 2012:
    ...
    datalist2 = pd.read_csv('2012.csv')      #create functiosn for 2 datasets
elif Year == 2015:
...
def plotanglechange(x, d, f):
    ...
    c.plot()
    plt.xlabel('Time')
    plt.ylabel('Angle of outward Field line from Normal')
    plt.title(d)
   
plt.figure()
plotanglechange(datalist, 'ACE', True)
plt.figure()
plotanglechange(datalist2, 'Juno', False)
...
plottingDaily(listDailyaverages, 'ACE')

#plottingHourly(listHourlyaverages2, 'Juno')
plottingDaily(listDailyaverages2, 'Juno')

#%%Processing,  24 NaN values (from 366),  and missing extra 28 values  8724, ignores everything beyond 364

lengthlimit = 6   #in hours, minimum length of sequence
Flucuations = 300

def reduceflucuations(x, d):
    s=[]
    c = []
    g =[]
    b= 0
    ...
    for i in x:
        ...
        elif b*i <= 0  and counter <= lengthlimit and Flag < Flucuations+1:               #else append equivalent in previous sequence 
            for j in np.arange(counter):
    ...
    return s, allsequences
            
plt.figure()       
listHourlyaverages['Final_angle'], histogramACE = reduceflucuations(listHourlyaverages['angle'], 'ACE')
#listHourlyaverages2['Final_angle'], histogramJuno  = reduceflucuations(listHourlyaverages2['angle'])
...
histogramplot(histogramACE, 'ACE')
histogramplot(histogramJuno, 'Juno')
...
#%%
#good matching for negative for 2015, not for positive
