...
period = 50
range1 = np.arange(1, 366, period)
range2 = np.arange(0, 366, 1)
...
def magav2(x):
    value= []
    for i in range1:        
            averaged = x.loc[i:i+period].mean()
            value.append(averaged)
...
ax[0].set_yticks(q)
ax[0] = ax[0].twinx()
ax[0].bar(range1, sun1['Dailysunspotno'],  width = period, alpha=opacity, color = 'Blue')
...
ax[1].set_yticks(q)
ax[1] = ax[1].twinx()
ax[1].bar(range1, sun2['Dailysunspotno'], width = period, alpha=opacity, color = 'Blue')
...
ax[2].bar(range1, sun3['Dailysunspotno'],width = period, alpha=opacity, color = 'Blue')
...
plt.plot(t['DECIMALDAY'], absolute(t['BX(nT)'], t['BY(nT)'], t['BZ(nT)']))
plt.xlabel('2016: Day')
plt.ylabel('|B|')
plt.ylabel('|B| nT')
'''
...
x = loadmat('Juno dist from Sun(1).fig')
