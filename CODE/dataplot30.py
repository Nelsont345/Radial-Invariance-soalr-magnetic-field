...
axs = datalist.hist('originalangle', bins=45 , normed=True)
datalist.hist('CorrectedAngle', bins=45, ax = axs , normed=True)
plt.legend('Defore COrrection', 'After correction')
plt.title('Frequency of angles of magnetic vector measurements')
...
plt.title('2016, ACE and Juno Daily Averages')
plt.ylabel("R^2 Br (nT AU^2)")
...
averagingperiodACE = 54/4
averagingperiodJuno = 50.6/4
...
def createdataset(averagingperiod, y, e):
    ...
    for i in x:        
        ...
        z2.append(positive)
        try:
            weightingneg = y['Magnitude_sign'].value_counts().loc[-1]
            weightingpos = e['Magnitude_sign'].value_counts().loc[1]
            negnumber = y['Magnitude_sign'].loc[i:i+averagingperiod].value_counts().loc[-1]/weightingneg
            posnumber =  e['Magnitude_sign'].loc[i:i+averagingperiod].value_counts().loc[1]/weightingpos
            print ('numbers', negnumber, posnumber) 
            print ('weight', weightingneg, weightingpos)
        except:
            continue
...
value1, days1 = magav(Daygroup1)
value2, days2 = magav(Daygroupminus1)
...
ax[0].set_title('ACE Postive Sector')
ax[0].plot(days1, value1)
ax[0] = ax[0].twinx()
ax[0].bar(np.arange(0,366, 1), data['Dailysunspotno'],  color = 'Blue')
#data.plot.bar(y = 'Dailysunspotno', ax =ax[0], sharex = True, sharey = True, color = 'Blue', legend = False)


ax[1].plot(days2, value2)   
ax[1].set_ylabel('                            R^2 Br (AU^2 nT)', color = 'Red')
ax[1].set_title('ACE Negative Sector')
ax[1].set_xlabel('Day')
ax[1] = ax[1].twinx()
ax[1].set_ylabel('                            Daily count', color = 'Blue')
ax[1].bar(np.arange(0,366, 1), data['Dailysunspotno'], color = 'Blue')
ax[0].get_shared_x_axes().join(ax[0], ax[1])
ax[0].set_xticklabels([])

#data.plot.bar(y = 'Dailysunspotno', ax =ax[1], sharey = True, color = 'Blue', legend = False)
